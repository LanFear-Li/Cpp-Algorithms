...
typedef struct A_expList_ *A_exp_list;
...
struct A_exp_ {
    enum {A_idExp, A_numExp, A_opExp, A_eseqEsp} kind;

    union {
        string id;
        int num;
        struct {A_exp left; A_binop binop; A_exp right;} op;
...
struct A_expList_ {
    ...
    union {
        struct {A_exp exp; A_exp_list exps;} pair;
        struct {A_exp exp;} last;
    } u;
};

A_exp_list A_PairExpList(A_exp exp, A_exp_list exps);
